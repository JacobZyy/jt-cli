...
JSON.parse(fs.readFileSync(base.slice(7), 'utf8'));
JSON.parse(fs.readFileSync(selected.slice(7), 'utf8'));
...
assert.equal(resolve(`?%${query.charCodeAt(0).toString(16)}${query.slice(1)}=${scenario}`), selected);
console.log('Whistle parser: default, scenario, method, exact path, parameter order, value boundaries, percent encoding and duplicate rejection passed');
