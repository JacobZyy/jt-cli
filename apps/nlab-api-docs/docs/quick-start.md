...
关联与闭合证明分开记录：`closed` 表示已证明完整字段值域；`known` 加 `enumAssociated: true` 表示已确认业务枚举归属，但不宣称数据库值域已被证明。源码枚举还必须满足 `primaryEnumValue: true`；已核实的响应字段注释可用 `enumAssociated: true` 生成枚举。操作级 TypeScript 类型和 OpenAPI 共用关联结果。同一主值经 `val()`、getter 或直接字段访问时使用统一身份。显式 null 分支保留可空类型。

该判定沿当前接口的请求、响应及 RPC 调用链；MyBatis 读取可通过已索引仓库中的同表映射、写入语句和 setter 追踪枚举来源，但不证明所有数据库写入。支持同值局部别名、多参数转发函数、DTO 复制和增强 for 循环中的枚举变量。不同对象、读取之间的值修改、冲突枚举或投影、未解析写入、算术变换以及枚举外默认值不建立类型关联。旧 IR 缺少主值确认标记时不会猜测收窄，重新 generate 可按新规则分析。`knownValues` 存在并不自动代表可关联。
...
注释和链接得到的候选按接口、请求/响应及字段路径核实，并在 `enumCandidate` 和生成报告中记录 `verified`、`conflict`、`unverified` 或 `ignored`。响应字段的注释值必须覆盖所有已解析写入值，允许多列未观测值；存在未解析写入或注释漏值时保留 `number` / `string`。链接枚举仍须独立的代码关联证据，使用已确认的源码主值；这不宣称枚举的每个成员都会出现在该响应中。整个过程由固定 Rust 规则执行，不调用 AI。
