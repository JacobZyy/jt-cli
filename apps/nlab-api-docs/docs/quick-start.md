...
关联与闭合证明分开记录：`closed` 表示已证明完整字段值域；`known` 加 `enumAssociated: true` 表示已确认业务枚举归属，但不宣称数据库值域已被证明。二者均可将枚举引用写入操作级 TypeScript 类型和 OpenAPI；关联字段附带 `x-nlab-enum-association` 元数据。显式 null 分支保留可空类型。

该判定仅沿当前接口的请求、响应及 RPC 调用链，不追查数据库所有写入路径。支持同值局部别名、多参数转发函数、DTO 复制和增强 for 循环中的枚举变量。不同对象、读取之间的值修改、冲突枚举或投影、未解析写入、算术变换以及枚举外默认值不建立类型关联。旧 IR 缺少新标记时保持原行为。`knownValues` 存在并不自动代表可关联。

DTO 来源追踪支持返回对象、参数转发、结果包装和 holder 的 getter/setter 中转；同一方法的多个调用实参无法证明为同一来源时保持开放。复杂集合映射、动态调用或未识别的包装可能仍只能导出独立枚举。字段改为枚举引用或显式可空类型后，业务代码中旧的宽泛类型、注释枚举和测试数据可能需要同步适配。
