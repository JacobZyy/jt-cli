  ...
  "operations": {
    "DemoFacade#detail": {
      ...
      "scenarios": {
        ...
        "done": {
          "values": { "/status": 20, "/buttons": [] },
          "sources": ["需求文档：完成场景无按钮"],
          "gaps": []
        }
      },
...
- `base`、`values` 和 `generators` 的键使用相对响应 data 的 JSON Pointer，例如 `/goods/title`、`/items/0/name`，不包含 `respData`。`~` 和 `/` 在属性名中分别写作 `~0`、`~1`。
- `base` 替换同一基础对象的字段；所有场景先克隆该对象，再应用 `values`。跨场景订单、商品和联系人不重新随机生成。跨接口需要共享的标识由各接口 `base` 固定为相同值。
...
| 输入 | 业务档位 | 输出 |
| --- | --- | --- |
| 没有已确认场景；只有契约或状态枚举 | 第一档 | 固定基础样例 |
| 有场景，声明部分覆盖或仍有缺口 | 第二档 | 已确认场景，共享基础对象，可用 Query 切换 |
| `coverage: complete`，有场景和来源，无声明或自动识别的缺口 | 第三档 | 本次约定范围内的完整场景 |

`complete` 声明要求有场景、来源且无声明缺口；自动生成仍发现未覆盖状态或按钮时，实际档位降到第二档并保留缺口。工具不核实外部文档的真实性，第三档表示规则声明范围及生成校验通过，不代替页面业务验收。

产物位于 `<output-root>/<appName>/`：

- `<Facade>/<method>.json`：原有默认样例路径。
- `<Facade>/<method>.base.json`、`<method>.<scenario>.json`：基础对象和场景响应。
- `whistle.rules`：静态路径、方法和 Query 选择。
- `coverage.json`：逐接口的生成结果、实际档位、输出路径、来源、假设、缺口，以及 seed、语言、参考日期、生成器版本和输入哈希。
...
载入生成的 `whistle.rules` 后，在目标接口 URL 加入开发 Query：

```text
/api/detail?__mock=pending
/api/detail?existing=1&__mock=done
/api/detail?__mock=base
```

Query 只控制静态样例，不修改请求类型。本轮开发约定是接口路径唯一、默认 POST、业务参数放在 body，URL 只携带一个 Mock 参数。直接按下面的原生规则生成（示例目录替换为真实项目绝对路径）：

```text
*/api/detail?__mock=qc-completed file://</Users/example/project/.nlab/mock/detail.qc-completed.json>
*/api/detail?__mock=refurbishing file://</Users/example/project/.nlab/mock/detail.refurbishing.json>
*/api/detail file://</Users/example/project/.nlab/mock/detail.json>
```

具体场景在前，默认最后。无需 HTTP 协议前缀、精确匹配前缀、方法过滤、includeFilter/excludeFilter、Query 正则或未知场景守卫。文件始终采用真实绝对路径；尖括号固定文件目标，避免追加剩余请求路径。找不到文件时返回本地 404。

本轮不处理 GET 业务查询参数、多 Query 顺序、特殊编码和其他未出现的输入需求。沿用 Whistle 原生前缀匹配与默认响应行为，不保证未知、空或重复场景返回 400；以后遇到真实需求再调整。JSON、三档策略、场景实体与文件保护不因此改变。

## 文件保护与验证

`.nlab/mock-manifest.json` 默认记录生成文件哈希。覆盖、删除陈旧文件、更新规则前统一检查；人工修改、未受管理的目标文件或路径跨 symlink 时拒绝执行。不要用 `--force` 绕过人工修改保护。
